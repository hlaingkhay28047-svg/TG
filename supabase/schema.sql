...
-- admin-adjustable profiles.allowed_devices, no longer one slot per type.
-- Existing databases carry the old one-per-type constraint under its
-- autogenerated name; drop it and keep lookups indexed.
alter table public.device_slots drop constraint if exists device_slots_user_id_slot_type_key;
create index if not exists device_slots_user_type_idx on public.device_slots (user_id, slot_type);
