...
const MOD_BITS = ['function refTile(tool, sel)', 'data-preset","__ref"', 'H.pickWire(tl, function(items){ setRefFiles(tool.id, items); }, "imRefFile")', 'H.pickWire(sw, function(items){ setRefFiles(tool.id, items); }, "imRefFile")',
  'if(ref && H.maxImages && H.maxImages(curModelId()) < 2){ H.toast(t("err_ref_model"),"warn"); return null; }', 'refDataUrl: g.ref ? g.ref.dataUrl : null', 'if(ref){ core = tool.refPrompt; if(desc) core += " EXTRA WISHES: " + desc; if(marked && tool.markPrompt) core += " " + D.frame.markRef; }', 'setRef:setRef, clearRef:clearRef,', 'grid.appendChild(refTile(tool, sel));'];   /* 6.32.0 — beside red paint the reference line gains the shared red-paint clause */
