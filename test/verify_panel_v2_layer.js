...
function releasePins() {
  ...
  report(`C2) the What's New strip carries the ${WAVE_V} row, in all nine languages, with a bold-led excerpt, and the panel's lifted table carries it (it led the strip when this wave shipped)`,
    !!row && wn.indexOf(row) <= 9 &&   /* 6.135.0, then 6.136.0 — one more release above it each time */ LANGS.every(l => row.t[l] && row.s[l] && !row.t[l].startsWith("**") && row.s[l].startsWith("**")) &&
    has(PWN, '"v":"' + WAVE_V + '"'), row && { at: wn.indexOf(row), kind: row.kind, ref: row.ref });

  report("C3) CI runs this test right after the skin-card check, and the suite counts 278 invocations (269 when this release shipped; 6.128.0 · 6.129.0 · 6.130.0 · 6.131.0 · 6.132.0 · 6.133.0 · 6.134.0 · 6.135.0 · 6.136.0 each added one)",
    ...
    (CI.match(/node test\//g) || []).length === 278 && has(LANDING, "278 tests"),
