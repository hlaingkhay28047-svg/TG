...
report("B5) the What's New strip carries the 6.124.0 row — kind wf, opening this card — a plain title and a bold-led excerpt in all nine languages, both naming the card, the excerpt naming the four modes; the panel's lifted table carries it (it led the strip when this wave shipped; the 6.125.0 row and every release since sit above it now)",
  !!row && rows.indexOf(row) <= 12 &&   /* 6.134.0, then 6.135.0 — one more release above it each time */ row.kind === "wf" && row.ref === ID && LANGS.every(l => row.t[l] && row.s && row.s[l] && !row.t[l].startsWith("**") && row.s[l].startsWith("**") &&
...
report("B6) CI runs this test right after the wave H check, the suite counts 277 invocations and the landing says 277 tests (266 until 6.125.0 added verify_album_wave_i, 267 until 6.127.0 added verify_skin_age_guard, 268 until 6.127.1 added verify_panel_v2_layer, 269 until 6.128.0 added verify_gen_loading_billing, 270 until 6.129.0 added verify_scene_frame_6129, 271 until 6.130.0 added verify_admin_wave_6130, 272 until 6.131.0 added verify_usage_ledger_6131, 273 until 6.132.0 added verify_scene_lock_agreement_6132, 274 until 6.133.0 added verify_horse_straw_6133, 275 until 6.134.0 added verify_panel_canvas_state_6134, 277 until 6.135.0 added verify_storage_clock_6135)",
  /run: PORT=8931 node test\/verify_prop_wave_h\.js\n(?:\s*#[^\n]*\n)*\s*- name: [^\n]*\n\s*run: PORT=8931 node test\/verify_selection_swap\.js/.test(CI) &&
  (CI.match(/node test\//g) || []).length === 277 && has(LANDING, "277 tests") && !has(LANDING, "265 tests") && /data-count="tests">277</.test(LANDING), { steps: (CI.match(/node test\//g) || []).length });
