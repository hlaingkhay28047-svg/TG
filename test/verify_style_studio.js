/* v6.12.0 — STYLE STUDIO: the RunningHub API_* AI Apps the owner showed, as cards
 * over the studio's own engines.
 *
 ...
 *
 * Usage: PORT=8931 node test/verify_style_studio.js  (serve docs/app first) */
"use strict";
const fs = require("fs");
...
const CAT = "Style Studio";
const IDS = ["anime-3d-portrait", "anime-2d-portrait", "ink-sketch", "manga-panel", "comic-to-real",
  "outpaint-extend", "watermark-clean", "character-sheet", "mecha-poster", "neon-street-night"];
/* the cards that hand back a PHOTOGRAPH of the person, and the ones that redraw them */
const PHOTO = ["comic-to-real", "outpaint-extend", "watermark-clean", "character-sheet", "mecha-poster", "neon-street-night"];
const REDRAW = ["anime-3d-portrait", "anime-2d-portrait", "ink-sketch", "manga-panel"];
...
(async () => {
  ...
  const A = await page.evaluate(({ cat, ids }) => {
    const cats = window.HNK_WF_CATALOG || [];
    const c = cats.find(x => x.t === cat);
    const items = c ? c.items : [];
    return {
      hasCat: !!c, order: items.map(w => w.id),
      badReq: items.filter(w => (w.req || []).length !== 1 || (w.opt || []).length !== 0).map(w => w.id),
      copies: ids.map(id => cats.reduce((n, k) => n + k.items.filter(w => w.id === id).length, 0))
    };
  }, { cat: CAT, ids: IDS });
  report("A) Style Studio is a category of its own with the ten cards, in the order shipped",
    A.hasCat && A.order.join(",") === IDS.join(","), A);
  report("A2) every card asks for exactly one photograph and no reference",
  ...
  report("C4) the six cards that hand back a photograph say REALISM; the four that redraw keep the person recognisable",
  ...
  await browser.close();
  console.log(failures ? `\n${failures} check(s) failed` : "\nAll checks passed — the Style Studio cards do what the apps they were modelled on do, over the studio's own engines.");
