...
(async () => {
  ...
  /* v5.13 — all four (Cinematic Poster, Nine-Tail Kitsune Fox, Mermaid
     ...
     none now. */
  /* v5.91.0 — the thirteen Look Sets ship their prompts first and their card
     art in the release that follows, which is the same two-step this pin was
     written for. Their pictures are generated on the showcase lane from each
     set's OWN shipped prompt (a card built from anything else would advertise
     a promise the button does not make), and that lane needs the owner's key
     as a masked dispatch input, so it cannot run from here.
     The pin keeps its teeth either way: any card NOT named below that falls
     back to the generated icon still fails this check, and each name comes
     off this list in the same commit that adds its jpg. */
  const EXPECT_SVG_ONLY = ["Butterfly Wing Light", "Golden Grecian", "Highland Indigo",
    "Gold Parasol Night", "Blossom Braids", "Sunflower Olive", "Chiaroscuro Cream",
    "Misty Grey Organza", "Marigold & Blue Veil", "Peach Velvet", "Temple Lightfall",
    "Noir Feather", "Imperial Yellow"];
