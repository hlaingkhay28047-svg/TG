...
function releasePins() {
  ...
  report(`E1) ${VER} / panel ${PVER} in lockstep: APP_VER, version.json, sw.js cache, API_VERSION, PANEL_VERSION, manifest, release-manifest (+ artifact file), panel-version.json, the download footer, the landing's badges`,
    has(APP, `var APP_VER="${VER}";`) && has(read("docs/app/version.json"), `"v":"${VER}"`) && has(read("docs/app/sw.js"), 'var CACHE = "hnk-web-studio-v6-118-0";') &&
    ...
    has(LANDING, VER) && has(LANDING, PVER) && !has(LANDING, "6.117.0") && !has(LANDING, "6.188.0"), { manifest: manifest.version, pv: pv.v });
  ...
  report("E3) CI runs this test (the 260th `node test/` invocation, right after the R2 + B2 step) and the landing says 260 tests",
    ...
    (CI.match(/node test\//g) || []).length === 260 && has(LANDING, "260 tests") && !has(LANDING, "259 tests"), { steps: (CI.match(/node test\//g) || []).length });
}
