...
(async () => {
  ...
  report("B1) app · image→video Result: the From chip shows the page's photo with the word, the direct link is a new-tab anchor to the clip, eight ways on (6.94.0: + Send to Upscale · Send to Video Tools; 6.95.0: + the two Frame chips), no takes strip with a single take",
    F.fromImg && F.fromWord === D.L.from[app.lang] && F.link && /hero-mermaid\.mp4$/.test(F.link) && F.target === "_blank" && F.navBtns === 8 /* 6.95.0 — + Send to Upscale · Send to Video Tools · Frame → Talking Photo · Frame → IMAGE 1 */ && F.takes === 0 && /hero-mermaid\.mp4$/.test(F.mainSrc), F);
  ...
  report("B4) app · video→video Result: the From chip names the clip, the clip plays, the direct link and eight ways on",
    app.v2v.fromWord.indexOf(D.L.fromClip[app.lang]) === 0 && /clip\.mp4$/.test(app.v2v.fromWord) && !!app.v2v.link && app.v2v.navBtns === 8 /* 6.95.0 */ && app.v2v.video, app.v2v);
  ...
  report("C2) panel · image→video Result with no player: no <video>, the vid_no_inline line for clip 1, one \"MP4 1\" tile marked, the From chip with the photo, the meta line (1080p · 5s), six ways on incl. the direct link and the two Send buttons",
    ...
    /1080p/.test(PI.meta) && /5s/.test(PI.meta) && PI.navBtns === 6 /* 6.165.0 — + the two Send buttons; 6.166.0 adds two Frame buttons only where <video> decodes, and this harness has no player */ && PI.link, PI);
  ...
  report("C5) panel · video→video Result: the wizard sits in #pageV2V, the take vtRun recorded is named as Renders/hnk-videotool-1.mp4, the From chip names the clip, the tool is on the meta line, seven ways on incl. Open the folder and the two Send buttons (shell.openPath gets the folder's path), and the page's result box is on behind it with its tile, saved line, no-player line, the app's heading and link words, Clear shown",
    ...
    /Tool X/.test(PV.meta) && PV.folderBtn && PV.navBtns === 7 /* 6.165.0 (no player here — see C2) */ && PV.note.length > 10 && PV.tiles.join() === "MP4 1" && PV.folderOpened === "/tmp/Renders" &&
