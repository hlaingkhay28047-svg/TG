...
const VER = "6.138.0", PVER = "6.209.0";   /* the wave shipped as 6.123.0 / 6.194.0; every release since moves the pair on, and the strip leads with the newest row */
...
/* ===================== B) the app's source ===================== */
report("B1) Background & Scene lists the prop and decor cards right after Reference Scenes and before Horse & Straw and Couple Compose; Repair & Enhance lists the gear card right after Selection Edit",
  has(APP, '"scene-fit-pro","studio-look-copy","reference-scenes","prop-insert","decor-theme-color","horse-straw","couple-compose",') && has(APP, 'st(["region-edit","selection-swap","light-gear-remove","upscale",'), null);   /* 6.124.0 — Selection Swap & Fill joined the row */
const sumGaps = [];
IDS.forEach(id => {
...
report("B4) the counts moved 194 → 197 → 198 → 199 Smart Workflows and 201 → 204 → 205 → 206 One-Tap (6.124.0 added Selection Swap & Fill, 6.133.0 added Horse & Straw) on the app's meta and statline fallbacks, the landing (ASCII and Myanmar digits, the two counters) and the panel's Home — and the old numbers are gone",
  (APP.match(/Smart Workflow 199/g) || []).length === 3 && !has(APP, "Smart Workflow 194") && (APP.match(/One-Tap 206/g) || []).length === 3 && !has(APP, "One-Tap 201") &&
  has(APP, '<b id="stWfCount">199</b>') && has(APP, '<b id="stTapCount">206</b>') &&
  (LANDING.match(/Smart Workflow 199/g) || []).length >= 30 && !has(LANDING_CLAIMS, "Smart Workflow 194") && (LANDING.match(/One-Tap 206/g) || []).length >= 30 && !has(LANDING_CLAIMS, "One-Tap 201") &&
  /data-count="wf">199</.test(LANDING) && /data-count="tap">206</.test(LANDING) && has(LANDING, "၁၉၉") && !has(LANDING_CLAIMS, "၁၉၄") &&
  has(PANEL_HOME, 'stat(206, "One-Tap Workflows");') && W.filter(x => !x.kind).length === 95,
  { app199: (APP.match(/Smart Workflow 199/g) || []).length, landing199: (LANDING.match(/Smart Workflow 199/g) || []).length, wf: W.length });
const artGaps = [];
IDS.forEach(id => {
...
report("B6) the What's New strip carries the 6.123.0 row — kind wf, opening the prop card — a plain title and a bold-led excerpt in all nine languages, both naming the three cards, and the panel's lifted table carries it (it led the strip when this wave shipped; the 6.123.1, 6.124.0, 6.125.0, 6.126.0 and every release since sit above it now)",
  !!row && row.v === WAVE_V && rows.indexOf(row) <= 17 &&   /* 6.134.0 through 6.137.0 — each release adds a row above it; the window widens by one rather than the row being re-dated */ row.kind === "wf" && row.ref === "prop-insert" && LANGS.every(l => row.t[l] && row.s && row.s[l] && !row.t[l].startsWith("**") && row.s[l].startsWith("**") &&
...
report("B7) CI runs this test right after the Reference Scenes check, the suite counts 280 invocations and the landing says 280 tests (265 until 6.124.0 added verify_selection_swap, 266 until 6.125.0 added verify_album_wave_i, 267 until 6.127.0 added verify_skin_age_guard, 268 until 6.127.1 added verify_panel_v2_layer, 269 until 6.128.0 added verify_gen_loading_billing, 270 until 6.129.0 added verify_scene_frame_6129, 271 until 6.130.0 added verify_admin_wave_6130, 272 until 6.131.0 added verify_usage_ledger_6131, 273 until 6.132.0 added verify_scene_lock_agreement_6132, 274 until 6.133.0 added verify_horse_straw_6133, 275 until 6.134.0 added verify_panel_canvas_state_6134, 276 until 6.135.0 added verify_storage_clock_6135, 277 until 6.136.0 added verify_storage_parity_6136, 278 until 6.137.0 added verify_album_light_6137, 279 until 6.138.0 added verify_album_all_6138)",
  ...
  (CI.match(/node test\//g) || []).length === 280 && has(LANDING, "280 tests") && !has(LANDING, "264 tests") && /data-count="tests">280</.test(LANDING), { steps: (CI.match(/node test\//g) || []).length });
...
const pGaps = [];
const SCENE_IDS = ["prop-insert", "decor-theme-color"];
IDS.forEach(id => {
  ...
  /* 6.129.0 — the app's Background & Scene post-pass adds three more house controls
     (Light match, Skin smooth, Balance) beside the v5.74.0 skin-tone switch, and the
     lifted copy carries them. The card's own fields are compared with the house keys
     set aside, and the two scene cards are asked for the house controls and locks. */
  const HOUSE_KEYS = ["skintone", "matchmode", "skinsmooth", "framebal"];
  const own = (it.fields || []).filter(f => HOUSE_KEYS.indexOf(f.key) < 0);
  if (JSON.stringify(own) !== JSON.stringify(w.fields)) pGaps.push(id + " fields differ");
  if (SCENE_IDS.indexOf(id) >= 0) {
    if (!["matchmode", "skinsmooth", "framebal"].every(k => (it.fields || []).some(f => f.key === k))) pGaps.push(id + " missing the 6.129.0 controls");
    if (it.prompt.indexOf("FRAME EXTENT LOCK:") < 0 || it.prompt.indexOf("COLOUR SEPARATION LOCK:") < 0) pGaps.push(id + " missing a 6.129.0 lock");
  }
...
report("C1) the panel's lifted catalog carries the three with the app's prompts, AVOID lists, inputs and own fields (plus the four house controls on the scene pair), nine card lines each, 199 cards in all — the scene pair under Background & Scene, the gear card under Repair & Enhance",
  pGaps.length === 0 && cat.total === 199 && items.length === 199 && !!bgCat && ["prop-insert", "decor-theme-color"].every(id => bgCat.items.some(x => x.id === id)) && !!rpCat && rpCat.items.some(x => x.id === "light-gear-remove"),
...
async function appWalk(browser) {
  ...
  const r = await page.evaluate((ids) => {
    ...
    out.bgOrder = bgIds.slice(bgIds.indexOf("reference-scenes"), bgIds.indexOf("reference-scenes") + 5);
...
(async () => {
  ...
  try {
    ...
    report("D1) the booted app composes 199 cards: Reference Scenes → Furniture & Prop Insert → Decor Theme Colour → Horse & Straw → Couple Compose under Background & Scene, Selection Edit → Selection Swap & Fill → Remove Light Stands & Gear under Repair & Enhance; the three carry their card pictures; the statline says 199 / 206",
      r.total === 199 && JSON.stringify(r.bgOrder) === JSON.stringify(["reference-scenes", "prop-insert", "decor-theme-color", "horse-straw", "couple-compose"]) && JSON.stringify(r.rpOrder.slice(0, 3)) === JSON.stringify(["region-edit", "selection-swap", "light-gear-remove"]) &&
      r.cardImg.every((c, i) => c === "lib/wf/cards5/" + IDS[i] + ".jpg") && r.titles[0] === "Furniture & Prop Insert" && r.wfc === "199" && r.tap === "206", r);
