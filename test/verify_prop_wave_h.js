...
const VER = "6.136.0", PVER = "6.207.0";   /* the wave shipped as 6.123.0 / 6.194.0; every release since moves the pair on, and the strip leads with the newest row */
...
report("B6) the What's New strip carries the 6.123.0 row — kind wf, opening the prop card — a plain title and a bold-led excerpt in all nine languages, both naming the three cards, and the panel's lifted table carries it (it led the strip when this wave shipped; the 6.123.1, 6.124.0, 6.125.0, 6.126.0 and every release since sit above it now)",
  !!row && row.v === WAVE_V && rows.indexOf(row) <= 15 &&   /* 6.134.0, 6.135.0, then 6.136.0 — each release adds a row above it; the window widens by one rather than the row being re-dated */ row.kind === "wf" && row.ref === "prop-insert" && LANGS.every(l => row.t[l] && row.s && row.s[l] && !row.t[l].startsWith("**") && row.s[l].startsWith("**") &&
...
report("B7) CI runs this test right after the Reference Scenes check, the suite counts 278 invocations and the landing says 278 tests (265 until 6.124.0 added verify_selection_swap, 266 until 6.125.0 added verify_album_wave_i, 267 until 6.127.0 added verify_skin_age_guard, 268 until 6.127.1 added verify_panel_v2_layer, 269 until 6.128.0 added verify_gen_loading_billing, 270 until 6.129.0 added verify_scene_frame_6129, 271 until 6.130.0 added verify_admin_wave_6130, 272 until 6.131.0 added verify_usage_ledger_6131, 273 until 6.132.0 added verify_scene_lock_agreement_6132, 274 until 6.133.0 added verify_horse_straw_6133, 275 until 6.134.0 added verify_panel_canvas_state_6134, 276 until 6.135.0 added verify_storage_clock_6135, 277 until 6.136.0 added verify_storage_parity_6136)",
  ...
  (CI.match(/node test\//g) || []).length === 278 && has(LANDING, "278 tests") && !has(LANDING, "264 tests") && /data-count="tests">278</.test(LANDING), { steps: (CI.match(/node test\//g) || []).length });
