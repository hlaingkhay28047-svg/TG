...
function releasePins() {
  ...
  /* 6.123.0 — this wave shipped as 6.122.0 / 6.193.0; every wave after it moves the pair on (the album
     designer's E1 says the same of 6.121.0). What stays true is the LOCKSTEP: one app version in every app
     file, one panel version in every panel file, the landing carrying both, and the pair at or past this wave's. */
  ...
  report(`E1) the release pair is in lockstep (this wave shipped as ${VER} / panel ${PVER}; the pair only moves forward): APP_VER, version.json, sw.js cache, API_VERSION agree; PANEL_VERSION, manifest, release-manifest (+ artifact file), panel-version.json agree; the download footer and the landing carry both`,
    ...
    has(LANDING, appV) && has(LANDING, panV) && !has(LANDING_CLAIMS, "6.121.0") && !has(LANDING_CLAIMS, "6.192.0"), { appV, panV, manifest: manifest.version, pv: pv.v });
  ...
  report("E3) CI runs this test right after the album designer and the landing says how many tests the suite runs (264 when this wave shipped, 265 since 6.123.0 added verify_prop_wave_h, 266 since 6.124.0 added verify_selection_swap, 267 since 6.125.0 added verify_album_wave_i, 268 since 6.127.0 added verify_skin_age_guard, 269 since 6.127.1 added verify_panel_v2_layer)",
    ...
    (CI.match(/node test\//g) || []).length === 269 && has(LANDING, "269 tests") && !has(LANDING, "263 tests"), { steps: (CI.match(/node test\//g) || []).length });
}
