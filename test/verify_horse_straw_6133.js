 ...
 * What's New row in nine languages, the CI step and the 275-test chain, and the 6.133.0 / 6.204.0
 * lockstep.
...
(async () => {
  ...
  try {
    ...
    const row = rows.filter(x => x.v === VER)[0];
    const PWN = read("panel/js/hnk_whats_new.js");
    report("D1) What's New leads with the " + VER + " row, in all nine languages, pointing at the card — and the panel's lifted table carries it too",
      !!row && rows.indexOf(row) === 0 && row.kind === "wf" && row.ref === ID &&
      LANGS.every(l => row.t[l] && row.t[l].length > 8 && row.s[l] && row.s[l].length > 200) &&
      has(PWN, '"v":"' + VER + '"') && has(PWN, '"ref":"' + ID + '"'),
    ...
    report("D4) " + VER + " / panel " + PVER + " in lockstep: APP_VER, version.json, sw.js cache, API_VERSION, PANEL_VERSION, manifest, release-manifest, panel-version.json, the download footer and the landing's badges",
      ...
      has(read("docs/app/sw.js"), 'hnk-web-studio-v6-133-0') &&
