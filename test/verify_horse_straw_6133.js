...
const VER = "6.138.0", PVER = "6.209.0";   /* the tree's current release, for the lockstep pin */
...
(async () => {
  ...
  try {
    ...
    report("D2) What's New carries the " + WAVE_V + " row near the top, in all nine languages, pointing at the card — and the panel's lifted table carries it too",
      !!row && rows.indexOf(row) <= 5 &&   /* 6.135.0, 6.136.0, then 6.137.0 — one more release above it each time */ row.kind === "wf" && row.ref === ID &&
