/* v6.13.0 — LENS STYLES: the named looks of RunningHub's Higgsfield Soul image
 * endpoint (higgsfield/soul/image-to-image, in the official HM-RunningHub
 * registry), rebuilt as photo-look cards over the studio's own engines.
 *
 * WHAT THE OWNER ASKED FOR: read RunningHub's official kits, find what the
 * studio does not carry yet, and add new Smart Workflow cards for images and
 * for video. The Soul endpoint's style list is a catalogue of camera looks —
 * a spotlight, an elevator mirror, a photo booth, 90s grain, a foggy morning,
 * through the glass, a red balloon, a giant accessory, a fisheye street, a
 * Y2K poster — and every one of them is a LIGHT-AND-PLACE change a shipped
 * edit engine performs from one photograph. The studio never invents an
 * endpoint client-side, so the looks ship as prompts. What is checked here is
 * what would quietly rot:
 *
 *   - the ten cards exist, once each, in Lens Styles, in the order shipped;
 ...
 *   - every card hands back a PHOTOGRAPH of the person and says so (REALISM)
 *     — a camera look that turns into a render has failed;
 *   - a Burmese summary in a Burmese UI; a 960x640 picture per card that
 *     renders on the Workflows page.
 *
 * Usage: PORT=8931 node test/verify_lens_styles.js  (serve docs/app first) */
"use strict";
const fs = require("fs");
...
const CAT = "Lens Styles";
const IDS = ["spotlight-stage", "elevator-mirror", "vintage-photobooth", "film-grain-90s", "foggy-morning",
  "through-the-glass", "red-balloon", "giant-accessory", "fisheye-street", "y2k-poster"];
/* every Lens Styles card hands back a PHOTOGRAPH of the person — a camera look, never a redraw */
const PHOTO = IDS.slice();
const REDRAW = [];
/* no card here takes a second picture */
const TWO_IN = {};
...
(async () => {
  ...
  report("A) Lens Styles is a category of its own with the ten cards, in the order shipped",
  ...
  await browser.close();
  console.log(failures ? `\n${failures} check(s) failed` : "\nAll checks passed — the Lens Styles cards hand back the camera looks they were modelled on, as photographs, over the studio's own engines.");
