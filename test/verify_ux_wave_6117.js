...
const sel = (pg) => `#${pg}.on:has(>.result-box.on)`;
...
function releasePins() {
  ...
  report(`D1) ${VER} / panel ${PVER} in lockstep: APP_VER, version.json, sw.js cache, API_VERSION, PANEL_VERSION, manifest, release-manifest (+ artifact file), panel-version.json, the download footer, the landing's badges`,
    has(APP, `var APP_VER="${VER}";`) && has(read("docs/app/version.json"), `"v":"${VER}"`) && has(read("docs/app/sw.js"), 'var CACHE = "hnk-web-studio-v6-117-0";') &&
    ...
    has(LANDING, VER) && has(LANDING, PVER) && !has(LANDING, "6.116.0") && !has(LANDING, "6.187.0"), { manifest: manifest.version, pv: pv.v });
  ...
  report("D3) CI runs this test (the 259th `node test/` invocation, right after the wave B3 + R1 step) and the landing says 259 tests",
    ...
    (CI.match(/node test\//g) || []).length === 259 && has(LANDING, "259 tests") && !has(LANDING, "258 tests"), { steps: (CI.match(/node test\//g) || []).length });
}
