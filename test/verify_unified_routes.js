...
check("landing advertises Web Studio 5.75.2", /Web Studio\s+(?:<[^>]+>)*v5\.75\.2/i.test(landing), "release copy is stale");
check("landing advertises Panel 6.48.0", /Panel(?:[^\n<]|<[^>]+>){0,80}v6\.48\.0/i.test(landing), "panel copy is stale");
check("web app reports 5.75.2", /var\s+APP_VER\s*=\s*["']5\.75\.2["']/.test(app), "APP_VER is stale");
