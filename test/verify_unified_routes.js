...
check("landing advertises Web Studio 5.50.3", /Web Studio\s+(?:<[^>]+>)*v5\.50\.3/i.test(landing), "release copy is stale");
check("landing advertises Panel 6.26.1", /Panel(?:[^\n<]|<[^>]+>){0,80}v6\.26\.1/i.test(landing), "panel copy is stale");
check("web app reports 5.50.3", /var\s+APP_VER\s*=\s*["']5\.50\.3["']/.test(app), "APP_VER is stale");
