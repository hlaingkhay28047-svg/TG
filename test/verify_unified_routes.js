...
check("landing advertises Web Studio 5.73.0", /Web Studio\s+(?:<[^>]+>)*v5\.73\.0/i.test(landing), "release copy is stale");
check("landing advertises Panel 6.45.0", /Panel(?:[^\n<]|<[^>]+>){0,80}v6\.45\.0/i.test(landing), "panel copy is stale");
check("web app reports 5.73.0", /var\s+APP_VER\s*=\s*["']5\.73\.0["']/.test(app), "APP_VER is stale");
