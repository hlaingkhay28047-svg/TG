...
check("landing advertises Web Studio 5.84.1", /Web Studio\s+(?:<[^>]+>)*v5\.84\.1/i.test(landing), "release copy is stale");
check("landing advertises Panel 6.56.1", /Panel(?:[^\n<]|<[^>]+>){0,80}v6\.56\.1/i.test(landing), "panel copy is stale");
check("web app reports 5.84.1", /var\s+APP_VER\s*=\s*["']5\.84\.1["']/.test(app), "APP_VER is stale");
