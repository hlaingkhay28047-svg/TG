...
check("landing advertises Web Studio 5.49.0", /Web Studio\s+(?:<[^>]+>)*v5\.49\.0/i.test(landing), "release copy is stale");
check("landing advertises Panel 6.25.3", /Panel(?:[^\n<]|<[^>]+>){0,80}v6\.25\.3/i.test(landing), "panel copy is stale");
check("web app reports 5.49.0", /var\s+APP_VER\s*=\s*["']5\.49\.0["']/.test(app), "APP_VER is stale");
