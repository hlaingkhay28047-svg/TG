...
check("landing advertises Web Studio 5.53.3", /Web Studio\s+(?:<[^>]+>)*v5\.53\.3/i.test(landing), "release copy is stale");
check("landing advertises Panel 6.28.2", /Panel(?:[^\n<]|<[^>]+>){0,80}v6\.28\.2/i.test(landing), "panel copy is stale");
check("web app reports 5.53.3", /var\s+APP_VER\s*=\s*["']5\.53\.3["']/.test(app), "APP_VER is stale");
