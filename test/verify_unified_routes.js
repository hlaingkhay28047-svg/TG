...
check("landing advertises Web Studio 5.47.1", /Web Studio\s+(?:<[^>]+>)*v5\.47\.1/i.test(landing), "release copy is stale");
check("landing advertises Panel 6.25.3", /Panel(?:[^\n<]|<[^>]+>){0,80}v6\.25\.3/i.test(landing), "panel copy is stale");
check("web app reports 5.47.1", /var\s+APP_VER\s*=\s*["']5\.47\.1["']/.test(app), "APP_VER is stale");
