...
function releasePins() {
  ...
  report("E3) CI runs this test right after the wave A floor and the landing says how many tests the suite runs (257 when this wave shipped, 258 since 6.116.0 added verify_ux_wave_6116)",
    ...
    (CI.match(/node test\//g) || []).length === 258 && has(LANDING, "258 tests") && !has(LANDING, "256 tests"), { steps: (CI.match(/node test\//g) || []).length });
}
