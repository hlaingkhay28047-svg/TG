...
function releasePins() {
  ...
  report(`E1) ${VER} / panel ${PVER} in lockstep: APP_VER, version.json, sw.js cache, API_VERSION, PANEL_VERSION, manifest, release-manifest (+ artifact file), panel-version.json, the download footer, the landing's badges`,
    has(APP, `var APP_VER="${VER}";`) && has(read("docs/app/version.json"), `"v":"${VER}"`) && has(read("docs/app/sw.js"), 'var CACHE = "hnk-web-studio-v6-115-0";') &&
    has(read("server/index.js"), `const API_VERSION = "${VER}";`) && has(read("panel/main.js"), `const PANEL_VERSION = "${PVER}";`) && has(read("panel/manifest.json"), `"version": "${PVER}"`) &&
    ...
    has(LANDING, VER) && has(LANDING, PVER) && !has(LANDING, "6.114.0") && !has(LANDING, "6.185.0"), { manifest: manifest.version, pv: pv.v });
  const rows = JSON.parse(WN.replace(/^window\.HNK_WHATS_NEW=/, "").replace(/;\s*$/, ""));
  const row = rows[0];
  report(`E2) the What's New strip leads with the ${VER} row — title and story in all nine languages, pointing at Home — and the panel's lifted table carries it`,
  ...
  report("E3) CI runs this test (the 257th `node test/` invocation, right after the wave A floor) and the landing says 257 tests",
    ...
    (CI.match(/node test\//g) || []).length === 257 && has(LANDING, "257 tests") && !has(LANDING, "256 tests"), { steps: (CI.match(/node test\//g) || []).length });
}
