...
function sourcePins() {
  ...
  report("A3b) the Imagine CSS: hub cards two to a phone row (the floor is 132px since 6.96.0 — it was 150px here, which gave a narrow Photoshop panel one column), three at 700px, four at 1100px, a compact body, a stacking foot, the rail as a sideways-scrolling row with margins (no gap), Model · Size above Apply; the panel's lifted copy carries the same rules and the same module lines. The summary ceiling moved two lines → five in 6.96.0 and the box became relative for the .ell marker — verify_ui_tidy_696 owns those two numbers now",
    /\.im-card\{flex:1 1 40%;min-width:132px;margin:5px;/.test(css) && /@media \(min-width:700px\)\{\.im-card\{flex:1 1 28%;min-width:0;max-width:calc\(33\.333% - 10px\)\}\}/.test(css) && /@media \(min-width:1100px\)\{\.im-card\{flex:1 1 20%/.test(css) &&
    ...
    /\.im-card-foot\{display:flex;flex-wrap:wrap;/.test(css) && /\.im-card-sum\{position:relative;margin:4px 0 8px;font-size:12px;line-height:1\.5;max-height:7\.5em;overflow:hidden/.test(css) && /\.im-msrow\{display:flex;flex-wrap:wrap;margin:0 -4px 10px\}/.test(css) &&
    ...
    PCSS.indexOf(".im-rail{display:flex;flex-direction:row;flex-wrap:nowrap;overflow-x:auto;overflow-y:hidden;") > 0 && PCSS.indexOf(".im-card{flex:1 1 40%;min-width:132px;margin:5px;") > 0, null);
  ...
  report("A5) Selection Edit on the web: openWizard forgets the rectangle per open, _wizRegion reads it, step 2 draws wizRegionPicker for region-edit, a required text field is the whole row, runWizGenerate crops IMAGE 1 to the rectangle before the run and pastes the result back after (the crop's gallery record replaced by the composite, IMAGE 1 restored)",
    ...
    /if\(\(w\.id==="region-edit"\|\|w\.region\) && state\.refs\[0\] && state\.refs\[0\]\.b64\) body\.appendChild\(wizRegionPicker\(state\.refs\[0\]\)\);/.test(APP) && /\+\(f\.required\?" wiz-field-req":""\)/.test(APP) &&
    /var rgn=\(\(w\.id==="region-edit"\|\|w\.region\) && wiz\.region && state\.refs\[0\] && state\.refs\[0\]\.b64\) \? wiz\.region : null, rgnFull=null;/.test(wiz)   /* 6.124.0 — the record's region flag joined the id */ && /var rgCrop=await wizRegionCrop\(state\.refs\[0\], rgn\);/.test(wiz) &&
...
function releasePins() {
  ...
  report("D1) the release: app " + appVer + " / panel " + panVer + " agree across version.json, APP_VER, docs/download/panel-version.json (v + latest_version) and the release manifest; CI runs this test; the landing counts at least 229 tests; What's New carries the " + appVer + " row on the app and the panel",
    ...
    /node test\/verify_ui_wave_693\.js/.test(CI) && (parseInt((LANDING.match(/data-count="tests">(\d+)</) || [])[1] || "0", 10) >= 229) && /* 6.107.0 — the table left index.html for data/whatsnew.js and the panel's
