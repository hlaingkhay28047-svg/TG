...
function sourcePins() {
  ...
  report("A3b) the Imagine CSS: hub cards two to a phone row (the floor is 132px since 6.96.0 — it was 150px here, which gave a narrow Photoshop panel one column), three at 700px, four at 1100px, a compact body, a stacking foot, the rail as a sideways-scrolling row with margins (no gap), Model · Size above Apply; the panel's lifted copy carries the same rules and the same module lines. The summary ceiling moved two lines → five in 6.96.0 and the box became relative for the .ell marker — verify_ui_tidy_696 owns those two numbers now",
    ...
    /\.im-card-foot\{display:flex;flex-wrap:wrap;/.test(css) && /\.im-card-sum\{position:relative;margin:4px 0 8px;font-size:11\.5px;line-height:1\.5;max-height:7\.5em;overflow:hidden/.test(css) && /\.im-msrow\{display:flex;flex-wrap:wrap;margin:0 -4px 10px\}/.test(css) &&
