...
async function appSkeleton(browser) {
  for (const [w, h] of [[390, 844], [1440, 900]]) {
    ...
    report(`B5) the skeleton at ${w}px: the spinner line is a live region; spinner on → the Freeform result card is on screen as a 9/16 skeleton (the picked ratio) with a shimmering picture box, aria-busy, its buttons hidden${wide ? ", beside the controls (two-pane)" : ""}; the picture lands in the skeleton's own frame (height within 4px)${wide ? " and the card does not move" : " and the card rises only by the closing status line"}; a second run over a finished picture is busy-only and keeps it; Video defaults 16/9, Text→Image 3/4; Retouch A is busy-only with its compare stage untouched`,
      ...
      /\bpending\b/.test(r.pending.cls) && r.pending.busy === "true" && r.pending.ar === "9/16" && r.pending.display === "block" && r.pending.imgAr === "9 / 16" && r.pending.imgH > 100 && !r.pending.dlVisible && r.pending.anim === "shimmer" && (wide ? r.pending.sideBySide : true) &&
...
function releasePins() {
  ...
  report(`E1) ${VER} / panel ${PVER} in lockstep: APP_VER, version.json, sw.js cache, API_VERSION, PANEL_VERSION, manifest, release-manifest (+ artifact file), panel-version.json, the download footer, the landing's badges`,
    has(APP, `var APP_VER="${VER}";`) && has(read("docs/app/version.json"), `"v":"${VER}"`) && has(read("docs/app/sw.js"), 'var CACHE = "hnk-web-studio-v6-120-0";') &&
    ...
    has(LANDING, VER) && has(LANDING, PVER) && !has(LANDING_CLAIMS, "6.119.0") && !has(LANDING_CLAIMS, "6.190.0"), { manifest: manifest.version, pv: pv.v });
  const rows = JSON.parse(WN.replace(/^window\.HNK_WHATS_NEW=/, "").replace(/;\s*$/, ""));
  const row = rows[0];
  report(`E2) the What's New strip leads with the ${VER} row — a bold lead, title and story in all nine languages, pointing at Home — and the panel's lifted table carries it`,
  ...
  report("E3) CI runs this test (the 262nd `node test/` invocation, right after the wave D step) and the landing says 262 tests",
    ...
    (CI.match(/node test\//g) || []).length === 262 && has(LANDING, "262 tests") && !has(LANDING, "261 tests"), { steps: (CI.match(/node test\//g) || []).length });
}
