...
report("B) the body script drives the app's own builders under a halting fetch stub and covers the three shelves",
  ...
  /if \(dump\.counts\.videoNoBody \|\| dump\.counts\.toolsNoBody \|\| dump\.counts\.imageNoBody \|\| errs\.length\) process\.exit\(1\);/.test(BODIES), null);
...
(async () => {
  ...
  report("D3) the probe's dry run reads every body, writes the key-free report and prints the marker block the container reads back",
    !!r && r.probed === (d ? d.counts.video + d.counts.tools + 1 + d.counts.image : -1) && r.ok === r.probed && !!marker && JSON.parse(marker[1]).probed === r.probed &&
