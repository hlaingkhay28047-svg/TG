...
function build(opts) {
  ...
  /* v6.92.0 — the app's block reads its tables from data/imagine.js (window.HNK_IMAGINE, loaded by the shell);
     the panel has no shell, so the same JSON is inlined here, byte for byte, where the app's line stands. */
  const DATA_LINE = "var IMAGINE_DATA = window.HNK_IMAGINE;";
  const mod0 = between(app, M0, M1, "module");
  if (mod0.split(DATA_LINE).length !== 2) throw new Error("build_panel_imagine: the IMAGINE module must read its tables with exactly one `" + DATA_LINE + "`");
  const mod = mod0.replace(DATA_LINE, "var IMAGINE_DATA = " + A.imagineText() + ";");
  const js = "/* GENERATED by tools/build_panel_imagine.js from docs/app/index.html — do not edit by hand.\n" +
    "   The web app's Imagine module (6.29.0 wave), byte for byte: IMAGINE_DATA (the tools, their\n" +
    "   templates in nine languages, the prompt frame — inlined from docs/app/data/imagine.js, which the\n" +
    "   web app loads by <script src> since 6.92.0) and the IMAGINE UI. main.js hands it the panel's\n" +
