...
--     id. Both columns reference public.hnk_auth_users, preserving integrity
--     without changing the live PostgREST relationship graph.
--
...
-- may leave behind: a small base64 image data URL or nothing. 98304 chars of
-- base64 is ~72KB of image, several times the app's real output, and small
-- enough that the profile row stays a cheap read.
...
/* 2026-08-30 owner instruction: how many machines an account may use is an
   ADMIN-ADJUSTABLE number, not a hard one-per-type rule. The old
   unique(user_id, slot_type) allowed exactly one computer + one phone
   forever; slots are now SEATS counted against profiles.allowed_devices
   (the column payments already multiply by), enforced in claimSlot under a
   per-user advisory lock. Existing single-slot rows are already valid
   seat rows, so this is purely additive for deployed data. */
alter table public.device_slots drop constraint if exists device_slots_user_id_slot_type_key;
create index if not exists device_slots_user_type_idx
  on public.device_slots (user_id, slot_type);
...
create table if not exists public.device_history (
  ...
  event_type     text not null check (event_type in ('registered','paired','seen','reset','blocked','limit')),
...
-- 2026-08-30 seat model: 'limit' records an admin adjusting allowed_devices.
-- Existing databases carry the old five-value check under its autogenerated
-- name; rebuild it (drop-then-add keeps this re-runnable).
alter table public.device_history drop constraint if exists device_history_event_type_check;
alter table public.device_history add constraint device_history_event_type_check
  check (event_type in ('registered','paired','seen','reset','blocked','limit'));
